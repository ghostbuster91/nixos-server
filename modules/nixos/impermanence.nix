...
in
{
  options.homelab.impermanence.persistZshHistory = mkOption {
    type = types.bool;
    default = true;
    description = "Persist the ${username} zsh history across reboots. Opt out on hosts where the home directory lives on a non-impermanent dataset.";
  };
  ...
  config = {
    ...
    environment.persistence."/persist" = {
      ...
      users = lib.mkIf config.homelab.impermanence.persistZshHistory {
        ${username} = {
          directories = [
